...
const aiwaAssetBase = window.aiwaTheme?.assetBase || "";

function resolveAsset(path) {
  if (!path || /^(https?:|data:|\/)/.test(path)) return path;
  return aiwaAssetBase ? `${aiwaAssetBase}${path}` : path;
}
...
const productData = window.aiwaProductData || fallbackProductData;
...
function renderProducts(category) {
  ...
  productGrid.innerHTML = data.products.map((product) => `
    <article class="product-card">
      <img src="${resolveAsset(product.image)}" alt="${product.type} ${product.name}">
      <div>
...
if (countryGrid && !countryGrid.children.length) {
